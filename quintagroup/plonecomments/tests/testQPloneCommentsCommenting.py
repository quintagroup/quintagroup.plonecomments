...
class TestMixinAnonymOn:
    ...
    def getDiscussionReplies(self, doc_obj):
        return self.discussion.getDiscussionFor(doc_obj).getReplies()
    ...
    def testAddCommentToDocAnonymUsers(self):
        ...
        replies_before = len(self.getDiscussionReplies(doc_obj))

        # Create talkback for document and Add comment
        self.logout()
        doc_obj.discussion_reply("Anonym reply", "text of 'anonym' reply")
        self.login('dm_admin')
        replies_after = len(self.getDiscussionReplies(doc_obj))
        self.failUnless(replies_after - replies_before,
        "Anonymous user CAN'T really add comment in terned ON *Anonymous "
        "commenting mode*.")
    ...
    def testAddCommentToDocNotAnonymUsers(self):
        ...
        for u in not_anonym_users:
            ...
            doc_id = "doc_%s" % u
            doc_obj = getattr(self.portal, doc_id)
            replies_before = self.getDiscussionReplies(doc_obj)
            self.login(u)
            ...
            self.login('dm_admin')
            replies_after = self.getDiscussionReplies(doc_obj)
...
class TestMixinAnonymOff:
    ...
    def afterSetUp(self):
        ...
        self.like_anonym = [u for u in all_users_id
...
class TestMixinModerationOn:
    ...
    def afterSetUp(self):
        ...
        if EnableAnonymComm:
            ...
            self.illegal_dm_users = [u for u in DM_USERS_IDS
                if not u in self.allowable_dm_users]
            self.illegal_common_users = [u for u in COMMON_USERS_IDS
    ...
    def testAddCommentToNotPublishedReplyDMUsers(self):
        ...
        for u in self.allowable_dm_users:
            ...
            reply = self.getDiscussionReplies(doc_obj)[0]
            ...
            replies_to_reply = self.getDiscussionReplies(reply)
            if not replies_to_reply:
                failed_users.append(u)
        self.failIf(failed_users,
        "%s - member(s) of DiscussionManager group CAN'T really "
        "ADD comment" % failed_users)
    ...
    def testAddCommentToNotPublishedReplyNotDMUsers(self):
        ...
        for u in self.allowable_common_users:
            ...
            doc_obj = getattr(self.portal, "doc_%s" % u)
            reply = self.getDiscussionReplies(doc_obj)[0]
            reply_to_reply = self.getDiscussionReplies(reply)
            reply_to_reply_before = len(reply_to_reply)
            self.logout()
            if not u=='anonym':
                self.login(u)
            # On adding reply to not published reply MUST generate
    ...
    def testAddCommentToPublishedReplyALLUsers(self):
        ...
        for u in allowable_users:
            ...
            if not reply_to_reply:
                failed_users.append(u)
        self.failIf(failed_users,
        "%s - user(s) can not really add comment "
        "to PUBLISHED reply." % failed_users)
...
class TestMixinModerationOff:
    ...
    def afterSetUp(self):
        ...
        if EnableAnonymComm:
            ...
            self.illegal_users = [u for u in self.all_users_id
    ...
    def testAddCommentToReplyAllowableUsers(self):
        ...
        for u in self.allowable_users:
            ...
            # Get reply to this document
            reply_to_doc = self.getDiscussionReplies(doc_obj)[0]

            # Create talkback for reply and Add comment
            replies_before = self.getDiscussionReplies(reply_to_doc)
            if not replies_before:
                self.discussion.getDiscussionFor(reply_to_doc)
            reply_to_doc.discussion_reply("%s's reply" % u,
                                          "text of '%s' reply" % u)
            replies_after = self.getDiscussionReplies(reply_to_doc)
            disparity = len(replies_after) - len(replies_before)
            if not disparity:
                failed_users.append(u)
        self.failIf(failed_users,
        "%s - member(s) CAN'T really ADD comment in terned "
        "off comments Moderation mode." % failed_users)
