...
class TestNotificationRecipients(FunctionalTestCase):
    ...
    def prepareRequest4Reply(self, member_id):
        ...
        membername = self.membership.getAuthenticatedMember().getUserName()
        self.request.form['Creator'] = membername
        self.request.form['subject'] = "Reply of '%s'" % membername
    ...
    def checkToANDSubj(self, mails, to, subj):
        ...
        self.failUnless([1 for m in messages
            if REXP_SUBJ.search(m) and REXP_SUBJ.search(m).group(1) == subj],
            "There is no message for '%s' recipient"
            " with '%s' subject" % (to, subj))
