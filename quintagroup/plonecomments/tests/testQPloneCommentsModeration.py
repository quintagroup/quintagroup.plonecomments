...
class TestModeration(FunctionalTestCase):
    ...
    def testViewRepliesNotPublishedNotDMUsers(self):
        ...
        authorized_users = [user for user in COMMON_USERS_IDS
                                 if user != 'anonym']
        users_without_md_perm = [u for u in authorized_users
    ...
    def testViewPublishButtonNonDMUsers(self):
        ...
        for u in users_without_md_perm:
            ...
            self.failIf(m,
                "Publish button present for %s - user "
                "without Moderate Discussion permission" % u)
    ...
    def testViewPublishButtonDMUsers(self):
        ...
        for u in DM_USERS_IDS:
            ...
            m = pattern.match(html)
            self.failUnless(m,
                "Publish button NOT PRESENT for %s - "
                "member of DiscussionManager group" % u)
    """
    ...
    def testPublishing(self):
        ...
        for u in DM_USERS_IDS:
            ...
            self.failUnless(getReplies(),
                "%s - member of DiscussionManager group "
                "NOT VIEW not published reply" % u)
            ...
            m = pattern.match(html)
            self.failIf(m,
                "Publish button present for %s - DiscussionManager"
    ...
    def testViewDeleteButtonNonDMUsers(self):
        ...
        for u in COMMON_USERS_IDS:
            ...
            if not u=='anonym' and 'Manager' in USERS[u]['roles']:
                self.failUnless(m,
                    "%s - user with Manager role NOT VIEW Delete reply button"
                    " for published reply on document view form" % u)
            else:
                self.failIf(m,
                    "%s - user without Manager role CAN VIEW Delete reply "
                    "button for published reply on document view form" % u)
    """
    ...
    def testDeleting(self):
        ...
        for u in managers:
            ...
            getReplies = self.discussion.getDiscussionFor(doc_obj).getReplies
            self.failUnless(getReplies(),
                "%s - user with Manager role not view discussion reply" % u)
            getReplies()[0].deleteDiscussion()
            self.failIf(getReplies(),
                "%s - user with Manager role not really delete discussion" % u)
