...
class TestConfiglet(FunctionalTestCase):
    ...
    def getDiscussionReplies(self, obj):
        return self.discussion.getDiscussionFor(obj).getReplies()
    ...
    def afterSetUp(self):
        ...
        self.portal.portal_membership.addMember('dm', 'secret',
                                                ['Manager'], [])
        portal_groups = getToolByName(self.portal, 'portal_groups')
        dm_group = portal_groups.getGroupById('DiscussionManager')
    ...
    def testSwitchONModeration(self):
        ...
        for u in DM_USERS_IDS:
            self.logout()
            self.login(u)
            self.failUnless(self.getDiscussionReplies(self.my_doc),
                "None discussion item added or discussion"
                " forbiden for %s user" % u)
        for u in COMMON_USERS_IDS:
            self.logout()
            if not u == 'anonym':
                self.login(u)
            noSecurityManager()
            self.failIf(self.getDiscussionReplies(self.my_doc),
                        "Viewing discussion item allow for Anonymous user")
    ...
    def testSwitchOFFModeration(self):
        ...
        # Create talkback for document and Add comment to my_doc
        self.discussion.getDiscussionFor(self.my_doc)
        authmember = self.portal.portal_membership.getAuthenticatedMember()
        self.request.form['Creator'] = authmember.getUserName()
        ...
        for u in all_users_ids:
            ...
            if not u == 'anonym':
                self.login(u)
            replies = self.getDiscussionReplies(self.my_doc)
            self.failUnless(replies,
                "No discussion item added or discussion"
    ...
    def testApproveNotification(self):
        ...
        self.portal.prefs_comments_setup()
        getProp = self.prefs.getProperty
        self.failUnless(getProp('enable_approve_notification') == 1,
        ...
        self.portal.prefs_comments_setup()
        self.failUnless(getProp('enable_approve_notification') == 0,
    ...
    def testPublishedNotification(self):
        ...
        self.portal.prefs_comments_setup()
        getProp = self.prefs.getProperty
        self.failUnless(getProp('enable_published_notification') == 1,
        ...
        self.portal.prefs_comments_setup()
        self.failUnless(getProp('enable_published_notification') == 0,
